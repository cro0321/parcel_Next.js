...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // console.log(data)
        setAllCarriers(data.data.Company)
        setCarriers(data.data.Company)
        // 완료되었을때 로딩끝
        setIsLoading(false);
  ...
  const PostSumbit = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://info.sweettracker.co.kr/api/v1/trackingInfo?t_code=${tcode}&t_invoice=${tinvoice}&t_key=${process.env.NEXT_PUBLIC_IP_API_KEY}`);
      const data = await res.json();
      console.log(data)
